...
import 'package:quran_viewer/src/data/search_result.dart';
...
Future<List<SearchResult>> search(String input) async {
  ...
  var request = http.Request(
    'GET',
    Uri.parse(
      'https://api.kalimat.dev//search?query=$input&getText=1&exactMatchesOnly=0&numResults=10&queryType=text',
    ),
  );
...
class QuranSearchBar extends StatefulWidget {
  const QuranSearchBar({super.key, required this.viewerController});
                ...
                (r) => ListTile(
                  ...
                  onTap: () {
                    if (r.type == 'quran_verse') {
                      final ayahId = r.id;
                      final page = widget.viewerController.pages
                          .firstWhereOrNull((p) => p.contains(ayahId));
                      if (page != null) {
                        widget.viewerController.pageController.jumpToPage(
                          page.id - 1,
                        );
                    ...
                    if (r.type == 'quran_chapter') {
                      final surah = widget.viewerController.surahs
                          .firstWhereOrNull((s) => s.id.toString() == r.id);
